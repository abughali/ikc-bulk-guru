...
def getAssetByName(client: CPDClient, project_id: str, name: str) -> str:
    ...
    if response.status_code != 200:
        raise ValueError(f"Error scanning project {project_id}: {response.text}")
    else:
...
def assignAssetOwner(client: CPDClient, project_id: str, asset_id: str, asset_name: str, owner_username: str) -> str:
    """
    ...
    url = f"/v2/assets/{asset_id}/collaborators?project_id={project_id}"
...
def updateAsset(client: CPDClient, project_id: str, asset_id: str, asset_name: str, display_name: str = None, 
               description: str = None, tags: List[str] = None, business_terms: List[Dict[str, str]] = None, 
...
def main(input_filename):
    ...
    with CPDClient() as client:
        ...
        print("\n" + "="*60)
        print("PROCESSING MULTI-PROJECT ASSET UPDATES")
        print("="*60)
        ...
        try:
            with open(input_filename) as csvfile:
                ...
                # Track projects being processed
                projects_processed = set()
                
                for row_num, row in enumerate(reader, 2):  # Start from 2 since we skipped header
                    if len(row) < 3:  # Ensure we have at least Project_ID, Project_Name, Asset columns
                    ...
                    tags_input = row[5].strip() if len(row) > 5 else ""
                    term1_name = row[6].strip() if len(row) > 6 else ""
                    term1_category = row[7].strip() if len(row) > 7 else ""
                    term2_name = row[8].strip() if len(row) > 8 else ""
                    term2_category = row[9].strip() if len(row) > 9 else ""
                    classification1_name = row[10].strip() if len(row) > 10 else ""
                    classification1_category = row[11].strip() if len(row) > 11 else ""
                    classification2_name = row[12].strip() if len(row) > 12 else ""
                    classification2_category = row[13].strip() if len(row) > 13 else ""
                    owner_username = row[14].strip() if len(row) > 14 else ""
                    
                    # Track projects being processed
                    projects_processed.add(f"{project_name} ({project_id})")
                    ...
                    print(f"\nProcessing row {row_num}: {project_name} > {asset_name}")
                    ...
                    try:
                        ...
                        if display_name or description or tags or business_terms or classifications:
                            update_status = updateAsset(client, project_id, asset_id, asset_name, display_name, description, tags, business_terms, classifications)
                        ...
                        if owner_username:
                            owner_status = assignAssetOwner(client, project_id, asset_id, asset_name, owner_username)
        ...
        try:
            with open(output_filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Write header
                header = ['Project_ID', 'Project_Name', 'Asset', 'Display_Name', 'Description', 'Tags', 
                         ...
                         'Owner', 'Metadata Update Status', 'Owner Update Status']
            ...
            print(f"Projects processed: {len(projects_processed)}")
        ...
        print("="*60)
        print("Multi-project asset update process completed.")
        print(f"Detailed results saved to: {output_filename}")


if __name__ == "__main__":
    # Expected CSV format with header (from export script):
    # Project_ID,Project_Name,Asset,Display_Name,Description,Tags,Term1 Name,Term1 Category,Term2 Name,Term2 Category,Classification1 Name,Classification1 Category,Classification2 Name,Classification2 Category,Owner
    # 
    # Where:
    # - Project_ID: ID of the project containing the asset
    # - Project_Name: Name of the project (for readability)
    ...
    main(input_filename='projects_assets.csv')