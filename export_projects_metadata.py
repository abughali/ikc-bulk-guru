...
def export_metadata():
    ...
    filename = f"exports/projects_metadata_{timestamp}.csv"
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as csv_file:
        ...
        # Write header
        writer.writerow(['Project ID', 'Asset Name', 'Column Name', 'Column Description', 
                         'Term Name', 'Term Category', 'Classification', 'Classification Category', 
                         'Classification2', 'Classification2 Category', 'Data Class Name', 'Data Class Category', 'Tags'])
                            ...
                            if asset_details and 'data_asset' in asset_details:
                                ...
                                for column in columns:
                                    try:
                                        ...
                                        # Extract existing term - lookup name and category by term_id
                                        term_name = ""
                                        term_category = ""
                                        ...
                                        if terms and len(terms) > 0:
                                            ...
                                            if term_id:
                                                term_name, term_category = lookup_term_by_id(term_id)
                                        ...
                                        if classifications:
                                            if len(classifications) > 0:
                                                ...
                                                if classification1_global_id:
                                                    classification1, classification1_category = lookup_classification_by_global_id(classification1_global_id)
                                            if len(classifications) > 1:
                                                classification2_global_id = classifications[1].get('global_id', '')
                                                if classification2_global_id:
                                                    classification2, classification2_category = lookup_classification_by_global_id(classification2_global_id)
                                        ...
                                        # Create populated row with existing metadata - names and categories looked up by IDs
                                        row = [
                                            project_id,               # Project ID
                                            asset_name,               # Asset Name  
                                            column_name,              # Column Name
                                            description,              # Column Description (populated)
                                            term_name,                # Term Name (looked up by term_id)
                                            term_category,            # Term Category (looked up by term_id)
                                            classification1,          # Classification (looked up by global_id)
                                            classification1_category, # Classification Category (looked up by global_id)
                                            classification2,          # Classification2 (looked up by global_id)
                                            classification2_category, # Classification2 Category (looked up by global_id)
                                            data_class_name,          # Data Class Name (looked up by data class id)
                                            data_class_category,      # Data Class Category (looked up by data class id)
                                            tags_string               # Tags (populated if exist)
                                        ]
                                        writer.writerow(row)
                                        total_columns += 1
    ...
    print(f"4. Save the file and use it as input for import_projects_metadata.py script")
